...
ytFormSearch?.addEventListener('submit', async (event) => {
  ...
  if (resultToSearch.items && resultToSearch.items.length > 0) {
    for (let i = 0; i < resultToSearch.items.length; i++) {
      ...
      const responseToStatistic = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${resultToSearch.items[i].id.channelId}&key=${youtubeKey}`)
